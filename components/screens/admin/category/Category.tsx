...
import DrawerForm from "./AddCategory"
...
const [isModOpen ,setIsModeOpen]=useState(false)
  useEffect(() => {
    fetchProducts()
  }, [])
  ...
  const fetchProducts = async () => {
    try {
      const response = await apiCall("/api/category")
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        {/* <Link href="/admin/category/AddCategory"> */}
          <Button onClick={()=>setIsModeOpen(true)}>
      ...
      <Card>
        <CardHeader>
          <CardTitle>All Category ({filteredProducts.length})</CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
        ...
        {
          isModOpen && <DrawerForm open={isModOpen} setOpen={setIsModeOpen}/>
        }
      </Card>
    </div>
  )
